<template>
  <v-card>
    <v-card-title>
      Update New Transfer Out Application
      <v-spacer></v-spacer>
    </v-card-title>
    <v-divider></v-divider>
    <v-card-text>
      ...
      <v-dialog
          v-model="gettingTransferOutDialog"
          persistent
          max-width="500"
      >
        <v-card>
          <v-card-text class="pa-5">
            <div class="text-center">
              <v-progress-circular
                  ...
                  indeterminate
              ></v-progress-circular>
              <h3 class="mt-5">Please wait while we get the Transfer Out Application Details...</h3>
            </div>
          </v-card-text>
        </v-card>
      </v-dialog>
      ...
      >
        <v-card>
          <v-card-title>
            Updating New Transfer Out Application
            <v-spacer></v-spacer>
            <v-icon
                small
          ...
          <v-row class="ma-0 pa-0">
            <v-col cols="4">
              <v-text-field
                  ...
                  dense
                  readonly
                  @keydown.enter.native="getTransferOut"
              ...
              <v-simple-table v-if="documents.length > 0">
                <template v-slot:default>
                  ...
                  >
                    ...
                    <td>
                      <v-btn icon color="success" text small class="mt-4"
                             v-if="isDocumentUploaded(document)"
    ...
    <v-card-actions class="pt-5 pb-5">
      <v-spacer></v-spacer>
      <v-btn color="primary" small dark @click="save">Update Transfer Out</v-btn>
    </v-card-actions>
  </v-card>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      files: {},
      gettingTransferOutDialog: false,
      unitCode: null,
      documentsError: null,
  ...
  computed: {
    ...
    progressMessage(){
      if(this.progressStage === 1){
        return "We are uploading the Transfer Out Documents..."
      }
      return "Please wait while the Transfer Out Application is getting Processed..."
    },
  ...
  methods: {
    getTransferOut() {
      ...
      this.gettingTransferOutDialog = true

      this.$store.dispatch("transferOutModule/getTransferOutApplication", this.$route.params.entityId)
          .then(res => {
            ...
            this.dateOfLeavingService = response.dateOfLeavingService
            this.dateOfTransferOut = response.dueDate
            this.alternateEmailId = response.altEmailId
            this.alternateMobileNumber = response.altContactNumber
            this.payeeName = response.payeeName
            this.addressLine1 = response.addressLine1
            this.addressLine2 = response.addressLine2
            this.addressLine3 = response.addressLine3
            this.addressLine4 = response.addressLine4
            ...
            this.fromPfNumber = response.fromPfNumber
            this.fromEpsNumber  = response.fromEpsNumber
            this.fromUanNumber  = response.fromUanNumber
            this.toPfNumber = response.toPfNumber
            this.toEpsNumber  = response.toEpsNumber
            this.toUanNumber  = response.toUanNumber
            this.currentEmployerName  = response.currentEmployerName
            this.currentEmployerAddressLine1  = response.currentEmployerAddressLine1
            this.currentEmployerAddressLine2  = response.currentEmployerAddressLine2
            this.currentEmployerAddressLine3  = response.currentEmployerAddressLine3
            this.currentEmployerAddressLine4  = response.currentEmployerAddressLine4
            ...
            this.selectedTransferOutType = response.transferOutType
            this.unitCode = response.unitCode
            this.gettingTransferOutDialog = false
            this.paymentModes = response.paymentModes
            this.selectedPaymentMode = response.paymentMode
            this.banks = response.banks
            this.selectedBank = response.paymentBank
            this.incomeTax = response.incomeTax
            this.educationCess = response.eduCess
          })
          .catch(error => {
            this.gettingTransferOutDialog = false
            this.error = error
          ...
          if (isValid) {
            ...
            this.uploadFiles()
                .then(response => {
                  ...
                  let data = {
                    "applicationId": this.$route.params.entityId,
                    ...
                    "fromEpsNumber": this.fromEpsNumber,
                    "fromUanNumber": this.fromUanNumber,
                    "toPfNumber": this.toPfNumber,
                    "toEpsNumber": this.toEpsNumber,
                  ...
                  return this.$store.dispatch("transferOutModule/updateTransferOutApplication", data)
    ...
    downloadFile(document){
      ...
      this.$store.dispatch("transferOutModule/downloadTransferOutDocument", path)
          .then(response => {
    ...
    isDocumentUploaded(document){
      return this.uploadedDocuments.filter(uploadedDocument => uploadedDocument.documentId === document.entityId).length > 0
    }
  },
  mounted() {
    this.getTransferOut()
