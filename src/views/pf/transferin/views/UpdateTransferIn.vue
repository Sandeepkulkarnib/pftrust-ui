<template>
  <v-card>
    <v-card-title>
      Update TransferIn Application
      <v-spacer></v-spacer>
    </v-card-title>
    <v-divider></v-divider>
    <v-card-text>
      <v-dialog
          v-model="gettingTransferInDialog"
          persistent
          max-width="500"
      >
        <v-card>
          <v-card-text class="pa-5">
            <div class="text-center">
              <v-progress-circular
                  ...
                  indeterminate
              ></v-progress-circular>
              <h3 class="mt-5">Please wait while we get the Transfer In Application Details...</h3>
            </div>
          </v-card-text>
        </v-card>
      </v-dialog>
      ...
      >
        <v-card>
          <v-card-text class="pa-5">
            ...
            <div v-show="!requestingTransferInApplication">
              <div style="display: block; margin-left: auto; margin-right: auto; width: 20%">
                <v-img width="70px" height="70px"
                       class="ma-5"
    ...
    <v-card-actions class="pt-5 pb-5">
      <v-spacer></v-spacer>
      <v-btn color="indigo" small dark @click="update">Update Transfer In</v-btn>
    </v-card-actions>
  </v-card>
</template>
...
import successCheckMark from "@/assets/success-green-check-mark.png"
...
export default {
  ...
  methods: {
    getTransferIn() {
      this.gettingTransferInDialog = true

      this.$store.dispatch("transferInModule/getTransferInApplication", this.$route.params.entityId)
          .then(response => {
            const data = JSON.parse(response)

            this.pernNumber = data.pernNumber;
            ...
            this.alternateEmailId = data.alternateEmailId
            this.prevPfNumber = data.prevPfNumber
            this.prevEpsNumber = data.prevEpsNumber
            this.prevAccountHolder = data.prevAccountHolder
            this.addressLine1 = data.addressLine1
            this.addressLine2 = data.addressLine2
            this.addressLine3 = data.addressLine3
            this.addressLine4 = data.addressLine4
            this.pfAccountNumber = data.pfAccountNumber
            this.epsAccountNumber = data.epsAccountNumber
            this.dateOfJoining = data.dateOfJoining
            this.unitCode = data.unitCode

            const previousCompany = data.previousCompany;

            this.companyName = previousCompany.companyName;
            this.companyDateOfJoining = previousCompany.companyDateOfJoining;
            this.companyDateOfLeaving = previousCompany.companyDateOfLeaving;
            this.prevAddressLine1 = previousCompany.addressLine1;
            this.prevAddressLine2 = previousCompany.addressLine2;
            this.prevAddressLine3 = previousCompany.addressLine3;
            this.prevAddressLine4 = previousCompany.addressLine4;

            this.gettingTransferInDialog = false
          })
          .catch(error => {
            console.log(error)
          })
    ...
    update(){
      ...
      if(isValid){
        ...
        this.$store.dispatch("transferInModule/updateTransferInApplication", data)
            .then(response => {
              const data = JSON.parse(response);
