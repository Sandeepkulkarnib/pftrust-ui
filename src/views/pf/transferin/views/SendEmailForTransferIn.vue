        ...
        >
          <template v-slot:top>
            <v-toolbar
                flat
            >
              <v-toolbar-title>Send Confirmation Emails for TransferIns</v-toolbar-title>
              <v-spacer></v-spacer>
              ...
              <v-card>
                ...
                <v-container fluid fill-height>
                  <v-row>
                    <v-col>
                      <div class="ma-3">
                        You have selected {{selected.length}} records for Sending TransferIn confirmation Emails.
                      </div>
                    </v-col>
                  </v-row>
                  <v-row>
                    <v-col>
                      <v-checkbox v-model="confirmSend" color="primary">
                        <template v-slot:label>
                          <div>
                            Please confirm that you want to send Transfer In Completion Emails.
                          </div>
                        </template>
                      </v-checkbox>
                    </v-col>
...
export default {
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'Posting Date', sortable: false, value: 'postingDate' },
        { text: 'Previous Company', sortable: false, value: 'prevCompany' },
      ],
      selected: [],
  ...
  methods: {
    getData(){
      this.loading = true
      this.$store.dispatch("transferInModule/fetchTransferInForEmailList")
          .then(response => {
            this.list = response
    ...
    sendEmails(){
      ...
      this.$store.dispatch("transferInModule/sendEmailsForTransferInCompletion", entityIds)
          .then(response => {
