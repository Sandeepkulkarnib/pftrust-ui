        ...
        >
          <template v-slot:top>
            <v-toolbar
                flat
            >
              <v-toolbar-title>Search Loans</v-toolbar-title>
              <v-text-field
                  hide-details
                  single-line
                  placeholder="search"
...
<!--              </v-btn>-->
              ...
              <approve-application-component
                  label="Approve Pending Loan Applications"
                  :dialog="dialog2"
                  @click:downloadTemplate="downloadTemplate"
                  @update:dialog="closeApproveApplicationDialog"
                  @load:applicationsData="loadApplicationsData"
                  :loading-applications="loadingPendingApplications"
                  :applications="pendingLoanApplications"
                  :headers="pendingLoanApplicationHeaders"
                  :total-applications="totalPendingLoanApplications"
                  @clear:applications="pendingLoanApplications = []"
              ></approve-application-component>
          ...
          <template v-slot:[`item.actions`]="{item}">
            <v-btn small color="primary" :to="'loans/details/' + item.entityId">
              view
            </v-btn>
          </template>
...
import ApproveApplicationComponent from "@/components/custom/ApproveApplicationComponent";

export default {
  name: "SearchLoan",
  components: {ApproveApplicationComponent},
  data () {
    return {
      totalLoans: 0,
      loans: [],
      ...
      headers: [
        ...
        { text: 'Reason Description', sortable: false, value: 'reason' },
      ...
      selectedYear: null,
      selectYearMonth: 1,
      dateType: ["Application Date", "Approval Date", "Payment Date"],
      selectedDateType: "Application Date",
      selectedLoanTypes: [],
      selectedLoanStatus: [],
      ...
      pendingLoanApplicationHeaders: [        {
        ...
        { text: 'Name', sortable: false, value: 'name' },
        { text: 'Loan Type', sortable: false, value: 'loanType' },
        { text: 'Payment Bank', sortable: false, value: 'paymentBank' },
        { text: 'Payment Mode', sortable: false, value: 'paymentMode' },
        { text: 'Cheque No.', sortable: false, value: 'chequeNo' },
        { text: 'Approval Date', sortable: false, value: 'approvalDate' },
        { text: 'Payment Date', sortable: false, value: 'paymentDate' },
        {text: 'Actions', sortable: false, value: 'actions'}]
    }
  },
  ...
  methods: {
    getLoans() {
      this.$store.dispatch("loanModule/fetchLoans", {
        ...
        search: this.search
          ...
          .then(response => {
            ...
            this.totalLoans = response.totalElements
            this.loading = false
          })
          .catch(error => {
            console.log(error)
    ...
    download(fileName){
      this.$store.dispatch("loanModule/downloadEmployeeReport", fileName)
          .then(response => {
    ...
    generateReport() {
      ...
      let data = {
        "params": JSON.stringify({
          ...
          "statusList": this.selectedLoanStatus,
          "types": this.selectedLoanTypes
        })
      }

      this.$store.dispatch("loanModule/startReportJob", data)
          .then(response => {
    ...
    trackJob(jobId, track) {
      this.$store.dispatch("loanModule/fetchJobDetails", jobId)
          .then(response => {
    ...
    downloadTemplate(){
      this.$store.dispatch("loanModule/downloadLoanApplicationTemplate",
          "ApprovePendingLoansTemplate")
          .then(response => {
    ...
    closeApproveApplicationDialog(value){
      this.dialog2 = value
    },
    loadApplicationsData(data){
      this.loadingPendingApplications = true

      console.log(JSON.stringify(data))

      let pernNumbers = data.applications.map((application) => application["Pern No"]);

      this.$store.dispatch("loanModule/getPendingLoanApplications", {
        "pernNumbers": pernNumbers
      })
          .then(res => {
            let response = JSON.parse(res)
            this.pendingLoanApplications = response.applications.map((loan) => {

              let found = data.applications.filter((application) => {
                return application["Pern No"] === loan["pernNumber"]
              })[0]

              if(loan.status === "success"){
                return {
                  "id": loan["id"],
                  "pernNumber": loan["pernNumber"],
                  "name": loan["name"],
                  "loanType": loan["loanType"],
                  "paymentBank": found["Payment Bank"],
                  "paymentMode": found["Payment Mode"],
                  "chequeNo": found["Cheque No"],
                  "approvalDate": found["Approval Date"],
                  "paymentDate": found["Payment Date"],
                  "status": loan["status"]
                }
              }else {
                return {
                  "id": loan["id"],
                  "pernNumber": found["Pern No"],
                  "name": "-",
                  "loanType": loan["loanType"],
                  "paymentBank": "-",
                  "paymentMode": "-",
            ...
            console.log(this.pendingLoanApplications)

            this.loadingPendingApplications = false
          })
          .catch(err => {
            console.log(err)
          })
  ...
  computed: {
    unitCodes () {
      return this.$store.state.loanModule.unitCodes.map(code => code.unitCode)
    },
    dateRangeText () {
    ...
    years() {
      return this.$store.state.loanModule.years
    },
    financialMonths() {
      return this.$store.state.loanModule.months
    },
    statusList() {
      return this.$store.state.loanModule.statusList
    },
    types() {
      return this.$store.state.loanModule.types
    },
    isLoansPending(){
      return this.loans.filter((loan) => loan.status === "P").length > 0
    },
    totalPendingLoanApplications(){
      return this.pendingLoanApplications.length
    },
  ...
  mounted() {
    ...
    if(this.unitCodes.length < 1){
      this.$store.dispatch("loanModule/fetchLoanEssentials")
