<template>
  <v-card>
    ...
    <v-card-actions class="pt-5 pb-5">
      <v-spacer></v-spacer>
      <v-btn color="primary" small dark @click="save">Create Loan</v-btn>
    </v-card-actions>
  </v-card>
</template>
...
export default {
  ...
  methods: {
    getDetails() {
      ...
      this.$store.dispatch("loanModule/getEmployeeDetails", this.pernNumber)
          .then(res => {
    ...
    getDetailsByPf() {
      ...
      this.$store.dispatch("loanModule/eligibleLoanTypesByPf", this.pfNumber)
          .then(res => {
            const response = JSON.parse(res)
            this.loadingEmployeeDetails = false
            this.pernNumber = response.pernNumber
      ...
      if(isValid){
        ...
        this.uploadFiles()
            .then(response => {
              ...
              return this.$store.dispatch("loanModule/createLoanApplication", data)
