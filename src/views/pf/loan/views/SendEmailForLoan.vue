        ...
        >
          <template v-slot:top>
            <v-toolbar
                flat
            >
              <v-toolbar-title>Send Confirmation Emails for Loans</v-toolbar-title>
              <v-spacer></v-spacer>
              ...
              <v-card>
                ...
                <v-container fluid fill-height>
                  <v-row>
                    <v-col>
                      <div class="ma-3">
                        You have selected {{selected.length}} records for Sending Loan confirmation Emails.
                      </div>
                    </v-col>
                  </v-row>
                  <v-row>
                    <v-col>
                      <v-checkbox v-model="confirmSend" color="primary">
                        <template v-slot:label>
                          <div>
                            Please confirm that you want to send Loan Completion Emails.
                          </div>
                        </template>
                      </v-checkbox>
                    </v-col>
...
export default {
  ...
  methods: {
    getData(){
      this.loading = true
      this.$store.dispatch("loanModule/fetchLoanEmailList")
          .then(response => {
            this.list = response
            this.loading = false
          })
    ...
    sendEmails(){
      ...
      this.$store.dispatch("loanModule/sendLoanCompletionEmails", entityIds)
          .then(response => {
