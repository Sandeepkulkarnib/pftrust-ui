<template>
  <v-card>
    ...
    <v-card-text>
      ...
      <v-dialog
          v-model="gettingSettlementDialog"
          persistent
          max-width="500"
      >
        <v-card>
          <v-card-text class="pa-5">
            <div class="text-center">
              <v-progress-circular
                  ...
                  indeterminate
              ></v-progress-circular>
              <h3 class="mt-5">Please wait while we get the Settlement Application Details...</h3>
            </div>
          </v-card-text>
        </v-card>
      </v-dialog>
    ...
    <v-card-actions class="pt-5 pb-5">
      <v-spacer></v-spacer>
      <v-btn color="primary" small dark @click="save">Update Settlement</v-btn>
    </v-card-actions>
  </v-card>
</template>
...
export default {
  ...
  methods: {
    getSettlement() {
      ...
      this.gettingSettlementDialog = true

      this.$store.dispatch("settlementModule/getSettlementApplication", this.$route.params.entityId)
          .then(res => {
            ...
            this.selectedSettlementType = response.settlementType
            this.dateOfLeavingService = response.dateOfLeavingService
            this.dateOfSettlement = response.dateOfSettlement
            ...
            this.gettingSettlementDialog = false
        ...
        if (isValid) {
          ...
          this.uploadFiles()
              .then(response => {
                ...
                return this.$store.dispatch("settlementModule/updateSettlementApplication", data)
    ...
    downloadFile(document){
      ...
      this.$store.dispatch("settlementModule/downloadSettlementDocument", path)
          .then(response => {
