        ...
        >
          <template v-slot:top>
            <v-toolbar
                flat
            >
              <v-toolbar-title>Send Confirmation Emails for Settlements</v-toolbar-title>
              <v-spacer></v-spacer>
              ...
              <v-card>
                ...
                <v-container fluid fill-height>
                  <v-row>
                    <v-col>
                      <div class="ma-3">
                        You have selected {{selected.length}} records for Sending Settlement confirmation Emails.
                      </div>
                    </v-col>
                  </v-row>
                  <v-row>
                    <v-col>
                      <v-checkbox v-model="confirmSend" color="primary">
                        <template v-slot:label>
                          <div>
                            Please confirm that you want to send Settlement Completion Emails.
                          </div>
                        </template>
                      </v-checkbox>
                    </v-col>
...
export default {
  ...
  methods: {
    getData(){
      this.loading = true
      this.$store.dispatch("settlementModule/fetchSettlementEmailList")
          .then(response => {
            this.list = response
            this.loading = false
          })
    ...
    sendEmails(){
      ...
      this.$store.dispatch("settlementModule/sendSettlementCompletionEmails", entityIds)
          .then(response => {
